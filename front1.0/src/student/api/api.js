...
export async function httpPost(path, body, requireAuth = false, method = 'POST') {
  ...
  try {
    ...
    if (!res.ok) {
      // 创建一个包含更多信息的错误对象
      const error = new Error(responseData.error || `${method} ${path} ${res.status}`);
...
export const api = {
  ...
  async getBookDetail(bookId) {
    try {
      ...
      // 4. 处理既不是主章节也不是子章节的章节（直接添加为独立章节）
      chapters.forEach(chapter => {
        // 检查章节是否已经被处理过
        const alreadyProcessed = mainChapters.some(mc => {
          return mc.id === chapter.id || mc.sections.some(s => s.id === chapter.id);
        });
  ...
  // 权限申请相关API
  async requestBookPermission(bookId, data) {
    try {
      console.log(`请求书籍权限: bookId=${bookId}`, data);
      return await httpPost(`/books/${bookId}/permission-request/`, data, true);
    } catch (error) {
      console.error('请求书籍权限失败:', error);
      throw error;
    }
  },
  ...
  async getAILearningGuide(bookId, chapterId) {
    try {
      console.log(`获取AI导学内容: bookId=${bookId}, chapterId=${chapterId}`);
      const response = await httpGet(`/books/chapters/${chapterId}/ai-learning-guide/`);
      console.log('AI导学内容API响应:', response);
      return response.content || null;
    } catch (error) {
      console.error('获取AI导学内容失败:', error);
      throw error;
    }
  },
  
  async generateAILearningGuide(bookId, chapterId) {
    try {
      console.log(`生成AI导学内容: bookId=${bookId}, chapterId=${chapterId}`);
      const response = await httpPost(`/books/chapters/${chapterId}/generate-ai-learning-guide/`, {});
      console.log('生成AI导学内容API响应:', response);
      return response;
    } catch (error) {
      console.error('生成AI导学内容失败:', error);
      throw error;
    }
  },
