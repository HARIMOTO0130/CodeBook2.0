  ...
  setup(props) {
    ...
    const loadAssignment = async () => {
      try {
        ...
        // 格式化作业数据
        formData.value = {
          title: assignmentData.homework_name || '',
          description: assignmentData.homework_content || '',
          total_score: assignmentData.total_score || 100,
          due_date: assignmentData.end_time ? new Date(assignmentData.end_time).toISOString().slice(0, 16) : '',
          classes: assignmentData.class_obj ? [assignmentData.class_obj] : []
        }
      } catch (error) {
    ...
    const submitAssignment = async () => {
      ...
      try {
        ...
        await assignmentApi.updateAssignment(props.id, data)
        alert('作业更新成功！')
        router.push(`/teacher/assignments/${props.id}`)
      } catch (error) {
        console.error('更新作业失败:', error)
        console.error('错误响应数据:', error.response?.data)
        console.error('请求配置:', error.config)
        alert('更新失败: ' + (error.response?.data?.error || error.response?.data?.non_field_errors?.[0] || error.message))
      } finally {
